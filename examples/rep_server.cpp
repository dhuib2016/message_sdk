...
msgsdk::Message handleDefault(const msgsdk::Message &msg) {
  ...
  std::cout << "[Server] 收到payload: " << msg.payload << std::endl;
...
int main() {
  ...
  auto server = std::make_shared<ZmqRepServer>(cfg);
  ...
  server->setMessageHandler(
      ...
      [server, &router](const msgsdk::Message &msg) mutable {
        ...
        reply.identity = msg.identity; // Copy ROUTER identity
  ...
  // std::cout << "Registered topics: user.login, order.create,
  // payment.process\n";
  //  获取所有已注册的 topic
