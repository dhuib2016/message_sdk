...
bool ZmqBase::start() {
    ...
    if (socketType() != zmqpp::socket_type::req) {
        ...
        wakeup_receiver_ = std::make_unique<zmqpp::socket>(context_, zmqpp::socket_type::pair);
        wakeup_sender_   = std::make_unique<zmqpp::socket>(context_, zmqpp::socket_type::pair);
